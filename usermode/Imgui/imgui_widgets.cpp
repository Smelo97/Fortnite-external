...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    if (*v)
    {
        ...
        //window->DrawList->AddRectFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2, check_bb.Min.y), check_bb.Max, GetColorU32(ImVec4(0.34f, 1.0f, 0.54f, 1.0f)), 0);
        window->DrawList->AddRectFilled(ImVec2(check_bb.Min.x + 0, check_bb.Min.y + 3), ImVec2(check_bb.Min.x + 18, check_bb.Min.y + 21), ImColor(160, 71, 255), 2);
    }
    else
    {
        window->DrawList->AddRectFilled(ImVec2(check_bb.Min.x + 0, check_bb.Min.y + 3), ImVec2(check_bb.Min.x + 18, check_bb.Min.y + 21), ImColor(41, 41, 41), 2);
    }

    if (label_size.x > 0.0f)
        RenderText(ImVec2(check_bb.Min.x + 23, check_bb.Min.y + 2), label);
    return pressed;
}
...
bool SliderBehaviorAAAAA(const ImRect& frame_bb, ImGuiID id, float* v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags)
{
    ...
    window->DrawList->AddCircleFilled(ImVec2(grab_bb.Min.x + 5, grab_bb.Min.y + 8), 4, ImColor(160, 71, 255), 128);
    window->DrawList->AddLine(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 10), ImVec2(grab_bb.Min.x, frame_bb.Min.y + 10), ImColor(160, 71, 255), 2);
...
void ImGui::ColorEditOptionsPopup(const float* col, ImGuiColorEditFlags flags)
{
    ...
    {
        if (allow_opt_inputs) Separator(1);
...
void ImGui::ColorPickerOptionsPopup(const float* ref_col, ImGuiColorEditFlags flags)
{
    ...
    {
        if (allow_opt_picker) Separator(1);
